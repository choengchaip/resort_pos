...
// Platform messages may fail, so we use a try/catch PlatformException.
  ...
  Future login()async{
    ...
    http.Response res = await http.post('${_authentication.GETPROTOCAL}://${_authentication.GETIP}:${_authentication.GETPORT}/APIs/login/login.php', body: userData);
    setState(() {
      isLoaded = true;
    });
    if(res.body != '0'){
      ...
      showDialog(
        ...
        builder: (BuildContext context){
          return AlertDialog(
            content: Text("ไม่พบผู้ใช้",style: _appFontStyle.getTopBarText(),),
            actions: <Widget>[
...
//                            hintText: languageData == null ? "Loading" : languageData['username']),
                            hintText: _languageServices.getText('username'),),
                      ),
                    ),
                    Container(
                      margin: EdgeInsets.only(bottom: 15),
                      ...
                      child: TextField(
                        ...
                        style: _appFontStyle.getInputText(),
                        decoration:
//                            InputDecoration.collapsed(hintText: languageData == null ? "Loading" : languageData['password']),
                            InputDecoration.collapsed(hintText: _languageServices.getText('password'),),
                      ),
                    ),
                    GestureDetector(
                      onTap: (){
...
//                                languageData == null ? "Loading" : languageData['fogetYouPassword'],
                                _languageServices.getText('fogetYouPassword'),
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                              ...
                              Expanded(
                                ...
                                child: Container(
                                  child: Text(
//                                    languageData == null ? "Loading" : languageData['email'],
...
//                                  languageData == null ? "Loading" : languageData['facebook'],
                                  _languageServices.getText('facebook'),
                                  style: _appFontStyle.getSmallButtonText(
                                      color: Color(0xffffffff)),
                                ),
                              ),
